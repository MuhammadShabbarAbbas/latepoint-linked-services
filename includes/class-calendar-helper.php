...
class OsLinkedServicesCalendarHelper {
     ...
     * This file just adds a new method to extract the data as JSON
     */

    /**
     * Extract booking data as JSON without generating the HTML picker
     *
     * @param OsBookingModel $booking The booking model
     * @param OsWpDateTime $target_date The target date
     * @param bool $auto_search Whether to auto search
     * @param array $calendar_settings Additional calendar settings
     *
     * @return array JSON-serializable data structure with dates and times
     */
    public static function extract_dates_and_times_data(OsBookingModel $booking, OsWpDateTime $target_date, array $calendar_settings = []) {
        ...
        // Create a booking request from the booking model
        ...
        $defaults = [
            ...
            'latest_possible_booking' => null,
            'earliest_possible_booking' => null
        ];

        $settings = OsUtilHelper::merge_default_atts($defaults, $calendar_settings);

        // Result data structure
        $result = [
            'allow_recurring' => $can_service_be_recurring,
            'time_pick_style' => OsStepsHelper::get_time_pick_style(),
            'show_booking_end_time' => OsSettingsHelper::is_on('show_booking_end_time'),
            'hide_unavailable_slots' => OsStepsHelper::hide_unavailable_slots(),
            'time_system' => OsTimeHelper::get_time_system(),
            'months' => []
        ];

        // Process target month
        $month_data = self::extract_month_data($booking_request, $target_date, [
            'active' => true,
        ...
        // Process additional preloaded months if needed
        for ($i = 1; $i <= $settings['number_of_months_to_preload']; $i++) {
            ...
            $month_data = self::extract_month_data($booking_request, $next_month_target_date, [
                'active' => false,
                'highlight_target_date' => false,
                'timezone_name' => $settings['timezone_name'],
                'exclude_booking_ids' => $settings['exclude_booking_ids'],
     ...
     * @param DateTime $target_date The target date for the month
     * @param array $settings Additional settings
     *
     * @return array Month data
     */
    private static function extract_month_data(\LatePoint\Misc\BookingRequest $booking_request, DateTime $target_date, array $settings = []) {
        $defaults = [
        ...
        if (!$service) {
            return [
                'error' => 'In order to generate the calendar, a service must be selected.'
            ];
        }
        ...
        // if it's a classic layout - it means we need to load some days from previous and next month
        ...
        $month_data = [
            ...
            'month_label' => OsUtilHelper::get_month_name_by_number($target_date->format('n')),
        ...
        for ($day_date = clone $calendar_start; $day_date <= $calendar_end; $day_date->modify('+1 day')) {
            ...
            $day_data = [
                ...
                'nice_date' => OsTimeHelper::get_nice_date_with_optional_year($day_date->format('Y-m-d'), false),
                'day_number' => $day_date->format('j'),
                'weekday' => $day_date->format('N'),
                'weekday_name' => OsBookingHelper::get_weekday_name_by_number($day_date->format('N')),
                'is_today' => $is_today,
                'is_past' => $is_day_in_past,
                'is_target_month' => $is_target_month,
                'is_next_month' => $is_next_month,
                'is_prev_month' => $is_prev_month,
                'is_in_allowed_period' => !$not_in_allowed_period,
                'service_duration' => $booking_request->duration,
                'total_work_minutes' => $total_work_minutes,
                'work_start_time' => $work_boundaries->start_time,
                'work_end_time' => $work_boundaries->end_time,
                'is_selected' => (($day_date->format('Y-m-d') == $target_date->format('Y-m-d')) && $settings['highlight_target_date']),
                'bookable_minutes' => $bookable_minutes,
                'bookable_slots_count' => count($bookable_minutes),
                'work_minutes' => $work_minutes,
                'selectable_interval' => $selectable_time_interval
            ];

            if ($service->is_group_service() && !$settings['hide_slot_availability_count']) {
                $day_data['available_capacity_by_time'] = $bookable_minutes;
            }

            // Get the bookable ranges information (only if the day is available and we have work minutes)
            if (!$is_day_in_past && !$not_in_allowed_period && $total_work_minutes > 0 && count($bookable_minutes) > 0) {
                $bookable_ranges = [];
                ...
                if ($bookable_ranges) {
                    $day_data['bookable_ranges'] = [];
                    for ($i = 0; $i < count($bookable_ranges); $i += 2) {
                        $start = $bookable_ranges[$i];
                        $end = isset($bookable_ranges[$i + 1]) ? $bookable_ranges[$i + 1] : $work_boundaries->end_time;

                        $day_data['bookable_ranges'][] = [
                            'start' => $start,
                            'end' => $end,
                            'percent_left' => (($start - $work_boundaries->start_time) / $total_work_minutes * 100),
                            'percent_width' => ((($end - $start) / $total_work_minutes) * 100)
                        ];
                    }
                }
            }

            $month_data['days'][] = $day_data;
        }

        return $month_data;
    }

    /* All the original methods would remain here... */
